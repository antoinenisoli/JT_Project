...
public class AutomaticGunScriptLPFP : MonoBehaviour {
	...
	private void Update () {
		...
		if (Input.GetButtonDown("KnifeAttack01") && !isInspecting) 
		{
			anim.Play("Knife Attack 1", 0, 0f);
		}
		//Play knife attack 2 animation when F key is pressed
		if (Input.GetButtonDown("KnifeAttack02") && !isInspecting) 
		{
			anim.Play("Knife Attack 2", 0, 0f);
		}
			
		//Throw grenade when pressing G key
		if (Input.GetButtonDown("ThrowProjectile") && !isInspecting) 
		{
		...
		//AUtomatic fire
		//Left click hold 
		if (Input.GetButton("Fire1") && !outOfAmmo && !isReloading && !isInspecting && !isRunning) 
		{
			//Shoot automatic
		...
		if (Input.GetButtonDown("Holster") && !hasBeenHolstered) 
		{
			holstered = true;
		...
		else if (Input.GetButtonDown("Holster") && hasBeenHolstered) 
		{
			holstered = false;
		...
		//Reload 
		if (Input.GetButtonDown("Reload") && !isReloading && !isInspecting) 
		{
			//Reload
			Reload ();
		}
